...
class ECurveTests: XCTestCase {
    ...
    func testAddInJacobian() {
        ...
        let p = UInt256(decimalString: "4294967189") // 2^32 - 107
        ...
        sum = curve[UInt256(decimalString: "2661831627"), UInt256(decimalString: "2993780686")]    // 3 * a
    ...
    func testDouble32Bit() {
        let p = UInt256(decimalString: "4294967189") // Careful: this will silently overflow: UInt256(4294967189)
        ...
        case let .Affine(x, y):
            ...
            XCTAssertTrue(y!.value ==  UInt256(decimalString: "2415436385"))
        default:
    ...
    func testAdd32Bit() {
        let p = UInt256(decimalString: "4294967189")
...
// Ambition:  < 1 second on iPhone 4S (currently 0.25 seconds on a MacBook Pro, 4 seconds on iPad Mini)
    ...
    func testMultiplyBig() {
        ...
        let a = UInt256(decimalString: "19898843618908353587043383062236220484949425084007183071220218307100305431102")
